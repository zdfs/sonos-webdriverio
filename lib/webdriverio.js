...
helpers = {
	...
	getChaiAsserts: function(assertMode, defaultAssert) {
		...
		var chaiAsserts = (assertMode) ? assertMode.split(' ') : defaultAssert;
...
drive = {
	...
	executeAction: function(args) {
		...
		switch (args.execute.action) {
			...
			case "redirect":
				testMethod = "url";
				testDesc = 'redirecting to ' + this.getBaseUrl() + args.execute.page;
				testArgs = [this.getBaseUrl() + args.execute.page];
				break;
			case "eval":
				testMethod = "execute";
	...
	verifyText: function(args) {
		...
		it('the "' + args.selector + '" element\'s text should read ' + '"' + args.assert + '"', function(done) {
